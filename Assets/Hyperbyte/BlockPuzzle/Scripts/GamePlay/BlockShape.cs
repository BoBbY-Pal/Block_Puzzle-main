...
namespace Hyperbyte
{
    ...
    public class BlockShape : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler
    {
        ...
        [HideInInspector] public SpriteType blockLayerImageType;
        private Block blockLayerHittedBlock;
        private Transform blockLayerActivatedChild = null;
        ...
        public void PrepareBlockShape()
        {
            ...
            if(blockLayerImageType != SpriteType.Empty)
            {
                if(transform.childCount > 0)
                {
                    blockLayerActivatedChild = activeBlocks[Random.Range(0, activeBlocks.Count - 1)].transform.GetChild(0);
                    Image blockLayerImage = blockLayerActivatedChild.GetComponent<Image>();
                    blockLayerImage.sprite = ThemeManager.Instance.GetBlockSpriteWithTag(blockLayerImageType.ToString());
            ...
            foreach (Transform t in activeBlocks)
            {
                ...
                if (hittingBlocks.Count == activeBlocks.Count)
                {
                    ...
                    for(int i = 0; i < hittingBlocks.Count; i++)
                    {
                        ...
                        if (blockLayerHittedBlock == block)
                        {
                            block.PlaceBlock(thisBlockSprite, spriteTag, blockLayerImageType);
                        } 
                        else
                        {
        ...
        Block GetHittingBlock(Transform draggingBlock)
        {
            ...
            {
                if(draggingBlock.transform.GetChild(0).GetComponent<Image>().enabled == true)
                {
                    blockLayerHittedBlock = hit.collider.GetComponent<Block>();
                }
