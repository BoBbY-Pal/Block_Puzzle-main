...
namespace Hyperbyte
{
    ...
    public class BlockShapesController : MonoBehaviour
    {
        ...
        private void Start()
        {
            if(GamePlayUI.Instance.currentLevel.SpecialBlockShape[0].allowSpecialBlockShape)
            {
                blockShapeCount = GamePlayUI.Instance.currentLevel.SpecialBlockShape[0].probability;
            }
        }
        /// <summary>
        ...
        void PrepareShapePool()
        {
            if (!hasInitialized)
            {
                ...
                */
                ...
                List<BlockShapeInfo> blockShapes = GamePlayUI.Instance.GetFixedBlockShapesInfo();
                foreach (BlockShapeInfo info in blockShapes)
                {
                    fixedBlockShape.Add(info.blockShape);
                    info.blockShape.GetComponent<BlockShape>().SetSpriteTag(info.blockSpriteTag);
                }
        ...
        void FillLastShapeContainer()
        {
            ...
            if (shapeContainer.blockShape == null)
            {
                ...
                if (GamePlayUI.Instance.currentLevel.SpecialBlockShape.Length > 0 && GamePlayUI.Instance.currentLevel.SpecialBlockShape[0].allowSpecialBlockShape &&
                    blockShapeCount >= GamePlayUI.Instance.currentLevel.SpecialBlockShape[0].probability)
                {
                    blockShapeCount = 0;
                    blockShape.blockLayerImageType = GamePlayUI.Instance.currentLevel.SpecialBlockShape[0].spriteType;
                }
